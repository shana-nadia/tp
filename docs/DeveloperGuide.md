...
The UI consists of a `MainWindow` that is made up of parts e.g.`CommandBox`, `ResultDisplay`, `LessonListPanel`, `StatusBarFooter` etc. All these, including the `MainWindow`, inherit from the abstract `UiPart` class which captures the commonalities between classes that represent parts of the visible GUI.
...
* depends on some classes in the `Model` component, as it displays `lesson` object residing in the `Model`.
...
1. The command can communicate with the `Model` when it is executed (e.g. to delete a lesson).<br>
...
<div markdown="span" class="alert alert-info">:information_source: **Note:** An alternative (arguably, a more OOP) model is given below. It has a `Tag` list in the `AddressBook`, which `lesson` references. This allows `AddressBook` to only require one `Tag` object per unique tag, instead of each `lesson` needing their own `Tag` objects.<br>
...
Step 2. The user executes `delete 5` command to delete the 5th lesson in the address book. The `delete` command calls `Model#commitAddressBook()`, causing the modified state of the address book after the `delete 5` command executes to be saved in the `addressBookStateList`, and the `currentStatePointer` is shifted to the newly inserted address book state.

![UndoRedoState1](images/UndoRedoState1.png)

Step 3. The user executes `add n/David …​` to add a new lesson. The `add` command also calls `Model#commitAddressBook()`, causing another modified address book state to be saved into the `addressBookStateList`.
...
| `*`      | disciplined tutor        | record my student's lessonality                                | know whether to bring my rattan cane                            |
...
1. Deleting a lesson while all lessons are being shown
   ...
   1. Prerequisites: List all lessons using the `list` command. Multiple lessons in the list.
