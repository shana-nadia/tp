...
public class EditCommandParserTest {
    ...
    public void parse_invalidPreamble_failure() {
        ...
        assertParseFailure(parser, "1 i/ string", "Unknown prefix: i/");

        // unknown prefix after valid fields
        assertParseFailure(parser, "1" + PHONE_DESC_AMY + " z/unexpected", "Unknown prefix: z/");
    }
    ...
    public void parse_invalidValue_failure() {
        ...
        assertParseFailure(parser, "1" + TAG_DESC_MATH + TAG_DESC_PHYSICS + TAG_EMPTY, Tag.MESSAGE_CONSTRAINTS);
        assertParseFailure(parser, "1" + TAG_DESC_MATH + TAG_EMPTY + TAG_DESC_PHYSICS, Tag.MESSAGE_CONSTRAINTS);
        assertParseFailure(parser, "1" + TAG_EMPTY + TAG_DESC_MATH + TAG_DESC_PHYSICS, Tag.MESSAGE_CONSTRAINTS);
        ...
        assertParseFailure(parser, "1" + INVALID_RATE_DESC, Rate.MESSAGE_CONSTRAINTS);
    }

    @Test
    public void parse_allFieldsSpecified_success() {
        Index targetIndex = INDEX_SECOND_PERSON;
        String userInput = targetIndex.getOneBased() + PHONE_DESC_BOB + TAG_DESC_PHYSICS
        ...
        EditPersonDescriptor descriptor = new EditPersonDescriptorBuilder().withName(VALID_NAME_AMY)
                ...
                .withRate(VALID_RATE_AMY).withTags(VALID_TAG_PHYSICS, VALID_TAG_MATH).build();
    ...
    public void parse_oneFieldSpecified_success() {
        ...
        descriptor = new EditPersonDescriptorBuilder().withDay(VALID_DAY_AMY).build();
        expectedCommand = new EditCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);

        // start time
        userInput = targetIndex.getOneBased() + START_TIME_DESC_AMY;
        descriptor = new EditPersonDescriptorBuilder().withStartTime(VALID_START_TIME_AMY).build();
        expectedCommand = new EditCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);

        // end time
        userInput = targetIndex.getOneBased() + END_TIME_DESC_AMY;
        descriptor = new EditPersonDescriptorBuilder().withEndTime(VALID_END_TIME_AMY).build();
        expectedCommand = new EditCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);
    ...
    public void parse_multipleRepeatedFields_failure() {
        ...
        userInput = targetIndex.getOneBased() + PHONE_DESC_AMY + ADDRESS_DESC_AMY + EMAIL_DESC_AMY
                ...
                + PHONE_DESC_BOB + ADDRESS_DESC_BOB + EMAIL_DESC_BOB + TAG_DESC_PHYSICS;
        ...
        // duplicate day
        userInput = targetIndex.getOneBased() + DAY_DESC_AMY + DAY_DESC_BOB;
        assertParseFailure(parser, userInput,
                Messages.getErrorMessageForDuplicatePrefixes(PREFIX_DAY));

        // duplicate start time
        userInput = targetIndex.getOneBased() + START_TIME_DESC_AMY + START_TIME_DESC_BOB;
        assertParseFailure(parser, userInput,
                Messages.getErrorMessageForDuplicatePrefixes(PREFIX_START));

        // duplicate end time
        userInput = targetIndex.getOneBased() + END_TIME_DESC_AMY + END_TIME_DESC_BOB;
        assertParseFailure(parser, userInput,
                Messages.getErrorMessageForDuplicatePrefixes(PREFIX_END));

        // duplicate rate
        userInput = targetIndex.getOneBased() + RATE_DESC_AMY + RATE_DESC_BOB;
        assertParseFailure(parser, userInput,
